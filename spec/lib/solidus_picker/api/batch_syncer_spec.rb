RSpec.describe SolidusPicker::Api::BatchSyncer do
  include ActiveSupport::Testing::TimeHelpers

  describe '.from_config' do
    it 'creates a syncer with the configured API client' do
      client = instance_double(SolidusPicker::Api::Client)
      allow(SolidusPicker::Api::Client).to receive(:from_config).and_return(client)
      shipment_matcher = -> {}
  ...
  describe '#call' do
    context 'when the API call is successful' do
      context 'when the sync operation succeeded' do
        it 'updates the Picker data on the shipment' do
          freeze_time do
            shipment = instance_spy('Spree::Shipment', number: 'H123456')
            api_client = instance_double(SolidusPicker::Api::Client).tap do |client|
            ...
            expect(shipment).to have_received(:update_columns).with(
              picker_order_id: '123456',
              picker_synced_at: Time.zone.now,
            )
          end
        end

        it 'emits a solidus_picker.api.sync_completed event' do
          ...
          expect(Spree::Event).to have_received(:fire).with(
            'solidus_picker.api.sync_completed',
            shipment: shipment,
            payload: {
              'orderNumber' => shipment.number,
              'success' => true,
      ...
      context 'when the sync operation failed' do
        it 'does not update the Picker data on the shipment' do
          shipment = instance_spy('Spree::Shipment', number: 'H123456')
          api_client = instance_double(SolidusPicker::Api::Client).tap do |client|
            allow(client).to receive(:bulk_create_orders).with([shipment]).and_return(
              {
        ...
        it 'emits a solidus_picker.api.sync_failed event' do
          stub_const('Spree::Event', class_spy(Spree::Event))
          shipment = instance_spy('Spree::Shipment', number: 'H123456')
          api_client = instance_double(SolidusPicker::Api::Client).tap do |client|
            allow(client).to receive(:bulk_create_orders).with([shipment]).and_return(
          ...
          expect(Spree::Event).to have_received(:fire).with(
            'solidus_picker.api.sync_failed',
            shipment: shipment,
            payload: {
              'orderNumber' => shipment.number,
              'success' => false,
    ...
    context 'when the API call hits a rate limit' do
      it 'emits a solidus_picker.api.rate_limited event' do
        stub_const('Spree::Event', class_spy(Spree::Event))
        shipment = instance_double('Spree::Shipment')
        error = SolidusPicker::Api::RateLimitedError.new(
        ...
        begin
          build_batch_syncer(client: api_client).call([shipment])
        rescue SolidusPicker::Api::RateLimitedError
          # We want to ignore the error here, since we're testing for the event.
        end

        expect(Spree::Event).to have_received(:fire).with(
          'solidus_picker.api.rate_limited',
          shipments: [shipment],
          error: error,
        )
      end
    ...
    context 'when the API call results in a server error' do
      it 'emits a solidus_picker.api.sync_errored event' do
        stub_const('Spree::Event', class_spy(Spree::Event))
        shipment = instance_double('Spree::Shipment')
        ...
        begin
          build_batch_syncer(client: api_client).call([shipment])
        rescue SolidusPicker::Api::RequestError
          # We want to ignore the error here, since we're testing for the event.
        end

        expect(Spree::Event).to have_received(:fire).with(
          'solidus_picker.api.sync_errored',
          shipments: [shipment],
          error: error,
        )
      end
