RSpec.describe SolidusPicker::Api::Client do
  describe '.from_config' do
    it 'generates a client from the configuration' do
      request_runner = instance_double('SolidusPicker::Api::RequestRunner')
      error_handler = instance_spy('Proc')
      shipment_serializer = instance_spy('SolidusPicker::Api::Serializer')
      allow(SolidusPicker::Api::RequestRunner).to receive(:from_config).and_return(request_runner)
      allow(SolidusPicker.config).to receive_messages(
  ...
  describe '#bulk_create_orders' do
    it 'calls the bulk order creation endpoint' do
      request_runner = instance_spy('SolidusPicker::Api::RequestRunner')
  ...
  def stub_shipment_serializer(results_map = {})
    serializer = class_spy('SolidusPicker::Api::Serializer')
